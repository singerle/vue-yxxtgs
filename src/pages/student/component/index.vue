<template>
  <div>
    <div class="bgcolor"
      ...
      <div class="weijiesuo"
           v-show="weijiesuo">
        <div class="text">请先完成入学准备后解锁</div>
      </div>
      <!-- 未完成入学准备点击到报道流程后，点击了app要显示的解锁提示遮罩 -->
      ...
      <div class="header">
        <div class="pic">
          <div class="picbg">
            <img :onerror="logo"
                 :src="info.tx==null||info.tx=='null'?'static/student-index-pictures/pic.png':info.tx">
          </div>
        </div>
        <div class="student-info">
          <h3 class="student-name">
            ...
            <span class="ico-1"
                  :style="info.xb === '男'?'background-image:url(static/student-index-pictures/man.png)':'background-image:url(static/student-index-pictures/woman.png)'"></span>
            <span class="ico-2"
                  @click="toShowCode"></span>
            <span class="ico-3"
                  @click="toDztzs"></span>
          </h3>
          <p class="yuan"
      ...
      <div class="contain">
        <div class="price"
          ...
          <h3>缴费信息</h3>
          <p class="need-price"
             :style="info.jfzt?'color:#3DA8F5':''">{{info.jfzt?'0.00':info.zfy}}</p>
          <p class="jfqk"
             style="padding-top:0.06rem">查看明细</p>
        </div>
        <div class="room"
             @click="toSzqs">
          <h3>所在寝室</h3>
          ...
          <p class="fpqk"
             style="color:#3DA8F5">查看详情</p>
        </div>
        <div class="tongzhishu"
             @click="toLuqu">
          <h3>录取通知书</h3>
          <p class="tzsqk">查看快递</p>
        </div>
      </div>
      <div class="tongzhi"
           @click="toNotice">
      ...
      <ul class="app-list"
        ...
        <li v-for="(item, index) in process"
            @click="togo(item.pauId, item.state, item.studentJumpPath)"
            :style="{'background-image':'url('+item.imgUrl+')','background-size': 'cover'}">
          <div><img v-show="item.state==0"
                 src="static/student-index-pictures/app-yiwancheng.png"></div>
          <p>{{item.eaaName}}</p>
        </li>
      </ul>
      <ul class="app-list app-list-href"
          v-show="type=='left'">
        <!--     		<div class="app-list-content" v-show="showHref">
    			<span class="app-list-ts">请先完成绿色通道后解锁</span>
    		</div> -->
        <li v-for="(item,i) in gdArr"
            @click="jump(item.path)"
            :style="{'background-image':'url('+item.logo+')','background-size': 'cover'}">
          <p>{{item.name}}</p>
        </li>
      </ul>
      <div class="footer-fixed"
           style="z-index:999">
        <div :class="{'left':type=='left'}"
             @click="changeApp('left')">
          <p>入学准备</p>
        </div>
        <div :class="{'right':type=='right'}"
             @click="changeApp('right')">
          <p>报到流程</p>
        </div>
      </div>
    </div>
    <div v-else-if="showYdy==1"
         class="home-ydy">
      <yindaoye @hiden="hiden"
                :data="ydyPicture"></yindaoye>
    </div>
