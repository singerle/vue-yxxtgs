<template>
  <div class="home-wrapper">
    ...
    <div class="home-content"
      ...
      <ul class="items">
        <li class="item"
            v-if="options.register&&options.register.flag">
          <div class="item-wraper border-left"
               @click="toPage('register', options.register.eaaRevoke)">
            <div class="icon register"></div>
            <div class="text">现场报到</div>
          </div>
        </li>
        <li class="item"
            v-if="options.channel&&options.channel.flag">
          <div class="item-wraper border-right"
               @click="toPage('channel', options.channel.eaaRevoke)">
            <div class="icon channel"></div>
            <div class="text">绿色通道</div>
          </div>
        </li>
        <li class="item"
            v-if="options.sushe&&options.sushe.flag">
          <div class="item-wraper border-left"
               @click="toPage('sushe', options.sushe.eaaRevoke)">
            <div class="icon sushe"></div>
            <div class="text">宿舍分配</div>
          </div>
        </li>
        <li class="item"
            v-if="options.insurance&&options.insurance.flag">
          <div class="item-wraper border-right"
               @click="toPage('insurance', options.insurance.eaaRevoke)">
            <div class="icon insurance"></div>
            <div class="text">保险购买</div>
          </div>
        </li>
        <li class="item"
            v-if="options.caiwu&&options.caiwu.flag">
          <div class="item-wraper border-left"
               @click="toPage('caiwu', options.caiwu.eaaRevoke)">
            <div class="icon caiwu"></div>
            <div class="text">财务缴费</div>
          </div>
        </li>
        <li class="item"
            v-if="options.manage&&options.manage.flag">
          <div class="item-wraper border-right"
               @click="toPage('manage', options.manage.eaaRevoke)">
            <div class="icon manage"></div>
            <div class="text">授权管理</div>
          </div>
        </li>
        <li class="item"
            v-if="options.news&&options.news.flag">
          <div class="item-wraper border-right"
               @click="toNew">
