        ...
        <el-row class="vue-header">
          ...
          <el-col :offset="7"
            ...
            <el-select size="small"
                       ...
                       @change="change()"
                       style="width:120px">
              <el-option v-for="(item) in allStateBtn"
                         :key="item.id"
                         :value="item.id"
                         :label="item.name">
              </el-option>
          ...
          <el-col :span="5">
            ...
            <el-select size="small"
                       ...
                       style="width:120px"
                       @change="change()">
              <el-option v-for="(item) in allYearBtn"
                         :key="item"
                         :label="item"
                         :value="item">
              </el-option>
        ...
        <el-table :data="tableData"
          ...
          <el-table-column prop="state"
            ...
            <template slot-scope="scope">
              <p>{{allStateBtn[scope.row.state]?allStateBtn[scope.row.state].name:'已结束'}}</p>
              <!-- <p>{{allStateBtn[scope.row.state].name}}</p> -->
            </template>
          </el-table-column>
          <el-table-column label="操作栏">
            <template slot-scope="scope">
              <el-button v-if="scope.row.state === 4 && roles[0] === '1'"
                         type="text"
                         size="small"
                         @click="test(scope.row)">测试</el-button>
              <el-button v-if="scope.row.state === 4 && roles[0] === '1'"
                         type="text"
                         size="small"
              ...
              <el-button v-if="scope.row.state === 4 && roles[0] === '1'"
                         type="text"
                         size="small"
                         @click="ruxueConfig(scope.row)">入学配置</el-button>
              <el-button v-if="scope.row.state === 3 && roles[0] === '1'"
                         type="text"
                         size="small"
                         @click="unable(scope.row)">停测</el-button>
              <el-button v-if="scope.row.state !== 4 && roles[0] === '1'"
                         type="text"
                         size="small"
                         @click="viewruxue(scope.row)">查看入学配置</el-button>
              <el-button v-if="scope.row.state !== 4 && roles[0] === '1'"
                         type="text"
                         size="small"
                         @click="viewSite(scope.row)">查看现场配置</el-button>
              <el-button v-if="scope.row.state === 4 && roles[0] === '1'"
                         type="text"
                         size="small"
                         @click="siteConfig(scope.row)">现场配置</el-button>
              <el-button type="text"
                         size="small"
                         @click="teacher(scope.row)">教师办公设置</el-button>
              <el-button v-if="scope.row.state === 4 && roles[0] === '1'"
                         type="text"
                         size="small"
