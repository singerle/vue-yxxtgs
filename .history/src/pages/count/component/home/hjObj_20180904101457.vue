<template>
    <div class="hjObj">
    	<header>
    		...
    		<span class="fr ft-12 my-color-gray" @click="togo(titleD)" v-if="titleD.name=='接站统计'||titleD.name=='新生男女比例'||titleD.name=='财务缴费'?false:true">更多</span>
    	</header>
    		...
    		<div class="hjObj-content-mes" v-if="titleD.name!='接站统计'">
    			<table>
	    			...
	    			<tr :class="{mt20:i==0}" v-for="(item,i) in data">
	    				<td>{{item.key}}</td>
	    				<td>{{item.bl}}</td>
	    				<td>{{((item.bl*10000)/(item.zrs*100))!=0? ((item.bl*10000)/(item.zrs*100)).toFixed(2):0}}%</td>
	    			</tr>
    			</table>
    		</div>
