    ...
    <div class="major">
			<table>
				...
				<tr v-for="item in get_xyData1">
					...
					<td>{{((item.sd*10000)/(item.zrs*100))!=0 ? ((item.sd*10000)/(item.zrs*100)).toFixed(2):0}}%</td>
				</tr>
			</table>
    </div>
</template>
...
export default {
  ...
  methods: {
      ...
      console.log(1)
  		this.set_xyData({func: 'getXybdqk',p:{xybh:encodeURI(this.xybh),sylx: encodeURI(this.get_sylx)}})
  	}
  }
}
</script>
